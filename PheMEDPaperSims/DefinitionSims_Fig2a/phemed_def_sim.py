...
n_gold = args.sample_size_gold
n_diluted = args.sample_size_diluted
...
df_alpha = pd.DataFrame(columns = ['Trial','Alpha','Method','ExpectedAlpha','Diluted_PPV','Diluted_NPV','Gold_PPV','Gold_NPV',
                                  'N_Gold','N_Diluted', 'N_Tests','P_Effects','Message','UsedBrute'])
alpha_index = 0
...
for trial in range(2000): #halve samples
    ...
    #Compute Dilution
    df_betas, df_ses = phe_help.extract_betas_ses(diluted_stats, gold_stats)
    ...
    if dilution < .1 or dilution > 10:
        ...
        used_brute = True
    df_alpha.loc[trial] = [trial, dilution, 'LL', expected_alpha, ppv_diluted, npv_diluted, ppv_gold, npv_gold, n_gold,
                                 n_diluted, n_tests, p_effects, optimizer.message, used_brute]
df_alpha.to_csv('data/phemed_validate_results' + str(job_id) + '.csv', index = False)
